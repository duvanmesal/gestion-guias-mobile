...
import { useDeletePuerto } from "../hooks/useDeletePuerto";
import { usePaisesLookup } from "../hooks/usePaisesLookup";
import { usePuertosList } from "../hooks/usePuertosList";
import type { CatalogStatus, PuertoListItem } from "../types/catalogs.types";
...
const PuertosPage: React.FC = () => {
  ...
  const { data, isLoading, isFetching, error, refetch } =
    usePuertosList(queryParams);
  const deletePuerto = useDeletePuerto();
  ...
  async function handleDelete(id: number, nombre: string) {
    const confirmed = window.confirm(
      `¿Seguro que deseas desactivar el puerto "${nombre}"?`
    );
    if (!confirmed) return;

    setFeedbackMessage(null);
    try {
      await deletePuerto.mutateAsync(id);
      setFeedbackMessage(`El puerto ${nombre} fue desactivado correctamente.`);
      ...
      <IonContent scrollY={true}>
        <div className="min-h-screen bg-[var(--color-bg-base)] px-5 pb-6 pt-8">
          <div className="mx-auto flex w-full max-w-md flex-col gap-4">
            <PageHeader title="Puertos" onBack={() => history.push("/admin/catalogos")} />
            ...
            <section className="rounded-2xl border border-[var(--color-border-hairline)] bg-[var(--color-bg-elevated)] p-4 shadow-[var(--shadow-card)]">
              <SectionHeader
                eyebrow="Filtros"
                title="Búsqueda"
                description="Busca por código, nombre o ciudad. Filtra por país y estado."
              />
              <form
            ...
            <section className="rounded-2xl border border-[var(--color-border-hairline)] bg-[var(--color-bg-elevated)] p-4 shadow-[var(--shadow-card)]">
              <div className="flex items-start justify-between gap-3">
                ...
                {isSuperAdmin ? (
                  <Button
                    ...
                    size="sm"
                    fullWidth={false}
                    onClick={() => history.push("/admin/catalogos/puertos/nuevo")}
                  >
                    Nuevo
                  </Button>
                ) : null}
              ...
              <div className="mt-4">
                {isLoading ? (
                  ...
                  <EmptyStateCard
                    title={hasFilters ? "Sin resultados" : "Aún no hay puertos"}
                    description={
                      hasFilters
                        ? "Prueba con otros filtros."
                        : "Cuando crees el primer puerto, aparecerá aquí."
                    }
                  />
                ) : (
                    ...
                    {items.map((puerto) => (
                      <CatalogRow
                        ...
                        details={[
                          ...
                          ["País", puerto.pais ? `${puerto.pais.codigo} · ${puerto.pais.nombre ?? ""}` : "—"],
                          ["Muelles", String(puerto._count?.muelles ?? 0)],
                        ]}
                        isSuperAdmin={isSuperAdmin}
                        isDeleting={
                          deletePuerto.isPending &&
                          deletePuerto.variables === puerto.id
                        }
                        onEdit={() => history.push(`/admin/catalogos/puertos/${puerto.id}`)}
                        onDelete={() => void handleDelete(puerto.id, puerto.nombre)}
                      />
                    ))}
                  </div>
                )}
