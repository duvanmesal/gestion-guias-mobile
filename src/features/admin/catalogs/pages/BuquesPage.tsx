...
const BuquesPage: React.FC = () => {
  ...
  const countriesMap = useMemo(() => {
    ...
    for (const country of countriesQuery.data ?? []) {
      map.set(country.id, `${country.codigo} · ${country.nombre}`);
    }
          ...
          <div className="mx-auto flex w-full max-w-md flex-col gap-4">
            ...
            <SurfaceCard className="gap-4 p-4" radius="xl" variant="raised">
              <PageSectionHeader
                ...
                description="Puedes buscar por código, nombre o naviera y filtrar por país de bandera y estado."
              />
            ...
            <SurfaceCard className="gap-4 p-4" radius="xl" variant="raised">
              <PageSectionHeader
                ...
                description={
                  data?.meta
                    ...
                    : "Listado administrativo del catálogo de buques"
                }
                action={
                  isSuperAdmin ? (
                    <Button
                      ...
                      fullWidth={false}
                      onClick={() => {
                        history.push("/admin/catalogos/buques/nuevo");
                      }}
                    >
                      Nuevo
                    </Button>
              ...
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {items.map((buque) => {
                    const countryLabel = buque.pais?.id
                      ? countriesMap.get(buque.pais.id) ||
                        `${buque.pais.codigo}${buque.pais.nombre ? ` · ${buque.pais.nombre}` : ""}`
                      : "Sin país";

                    const subtitleParts = [
                      buque.codigo,
                      countryLabel,
                      buque.naviera ? `Naviera: ${buque.naviera}` : null,
                      typeof buque.capacidad === "number"
                    ...
                    return (
                      <EntityListRow
                        ...
                        metadata={
                          <StatusChip
                            tone={
                              buque.status === "ACTIVO" ? "success" : "warning"
                            }
                            dot
                          >
                            {buque.status === "ACTIVO" ? "Activo" : "Inactivo"}
                          </StatusChip>
                        }
                        action={
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              onClick={() => {
                                history.push(
                                  `/admin/catalogos/buques/${buque.id}`
                                );
                              }}
                            >
                              Editar
                            ...
                            {isSuperAdmin ? (
                              <Button
                                ...
                                onClick={() => {
                                  void handleDelete(buque.id, buque.nombre);
                                }}
                              >
                                Desactivar
                              </Button>
